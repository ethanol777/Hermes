# OpenViking (GPS Tool) Install on WSL Without Sudo
...
3. sudo is disabled globally — can't even do `sudo -n`; confirmed with `/etc/sudoers` check
4. WSLg installed but `DISPLAY` was empty — manually set to `:0`

...
### 🔴 `patch` 工具在 UTF-8 中文 JSONL 文件上会失败（2026-05-16 发现）

**现象：** 用 `patch(action='patch', old_string=..., new_string=...)` 向 `fact_store.jsonl` 追加新行时，如果 `old_string` 或 `new_string` 包含中文字符，`patch` 会返回「Could not find a match for old_string」错误——即使内容完全匹配。因为 `patch` 内部对包含 CJK 字符的 JSONL 条目的匹配存在编码问题。

**影响：** `fact_store.jsonl` 的文件操作（append）不能依赖 `patch`。

**正确做法：**

```
✅ fact_store(action='add')       ← 优先使用 tool 写入（处理 dedup/trust）
✅ fact_store(action='add') 再试  ← 如果第一次失败，重试（可能是瞬时问题）
⚠️ terminal echo '...' >> file    ← 备选：跳过 trust 机制但能落地
❌ patch()                         ← 对中文 JSONL 不可靠
```

**2026-05-16 事故流程：**
1. read_file 正确显示了 fact_store.jsonl 末尾内容
2. patch() 用完全相同的文本匹配——失败
3. 再用 echo >> 写入——成功但跳过了 fact_store tool 的 trust 评分
4. **隐患：** 如果用 echo >> 写入了大量重复事实，后续 `fact_store` 去重可能失效

**教训：** 写入 fact_store 的首选永远是 `fact_store(action='add')`。只有在 tool 本身不可用（报错）时才走文件直写路线。不要因为「patch 在通用场景好用」就以为它在所有文件上都能工作——JSONL 是编码敏感的。
...
**更新时必须两个都写，否则下次会话读到旧版本。** 最佳实践顺序：

```bash
# 1. 先写主副本（AppData）
write_file("C:\\Users\\77\\AppData\\Local\\hermes\\memories\\MEMORY.md", content)

# 2. 再同步到副副本（Hermes 目录）
write_file("C:\\Users\\77\\Hermes\\hermes\\memories\\MEMORY.md", content)
```

**⚠️ 这个操作太容易忘，所以强制规则：写完第一个副本之后，立刻——不是稍后、不是最后再统——去找第二个副本路径。等"最后再写"意味着永远不会写。**

**2026-05-16 反例（本轮会话）：** 我只写了 Hermes 目录的副本（因为 path 在 APP_CONTEXT 里被我读到了），没写 AppData 主副本（因为路径不在 APP_CONTEXT 里，我忘了创建或去查）。下次会话——如果读的是 AppData 路径——这轮全部学习内容都会丢失。

**防错方法：** 在 `write_file` 之前，先列出两个路径，强迫自己看到它们：
```bash
# 1. 列出两个路径（看着它们写）
# 写第一个：
C:\Users\77\AppData\Local\hermes\memories\MEMORY.md ← 先写这个
# 写第二个：
C:\Users\77\Hermes\hermes\memories\MEMORY.md    ← 马上写这个

# 2. 如果其中一个路径不确定是否存在，用 terminal 的 mkdir -p 确保目录存在
# 3. 写完后 grep -c "auto-learned" 两个文件确认行数一致
```

...
```
✅ fact_store(action='add')       ← 优先使用 tool 写入（处理 dedup/trust）——需要确认 tool 在不在
✅ terminal echo '{"id":"fs_NNN","fact":"...","tags":"...","confidence":0.85}' >> fact_store.jsonl  ← 直接 JSONL 追加
   ⚠️ 注意：Windows 下的 echo 用双引号包围 JSON，外层用单引号给 bash
   ⚠️ 写完后 read_file 确认最后一行是完整 JSON（没有因 shell 转义被截断）
⚠️ write_file 直接写 fact_store.jsonl 内容  ← 需读-改-写三步，可能丢并发
❌ patch()                         ← 对中文 JSONL 不可靠

# fact_store.jsonl 也有两个副本，必须都写：
echo '...' >> /c/Users/77/Hermes/hermes/memories/fact_store.jsonl
echo '...' >> /c/Users/77/AppData/Local/hermes/memories/fact_store.jsonl
```

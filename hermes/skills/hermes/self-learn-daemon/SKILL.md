...
### 🔴 `patch` 工具在包含中文/长段重复模式的文件上会失败（2026-05-16 发现，2026-05-16 扩展）

**现象：** 用 `patch(action='patch', old_string=..., new_string=...)` 向任何包含中文字符或多处相同模板化字符串的文件做追加时都会失败：
- `fact_store.jsonl` 追加新行 → 「Could not find a match」，因为 CJK 字符造成编码匹配偏差
- `MEMORY.md` 尾部追加 → 「Found N matches」（N≥2），因为多段 auto-learned 条目共享完全相同的模板片段（`|- Platform:`、`|- Source:`、`- Insight:` 等），而 `patch` 要求唯一匹配

**根因：** 不是文件格式的问题（不是「JSONL 编码敏感」），是两件事：
1. **CJK 编码匹配偏差：** `patch` 工具对含中文字符的 `old_string` 的字节级匹配可能走偏移（适用于所有文件，不限于 JSONL）
2. **模板化重复：** auto-learned 条目的 markdown 模板高度一致，`patch` 用简单字符串匹配时找到多个位置

**影响：** MEMORY.md 和 fact_store.jsonl 的追加操作都不能依赖 `patch`。

**正确做法（按优先级排序）：**

| 方法 | 适用场景 | 注意事项 |
|------|---------|---------|
| `write_file` 全量重写 | MEMORY.md 追加多段内容 | 先 read_file 取得全文，追加内容后 write_file 写回。安全但可能丢并发。文件较大时可行（本次实测 4KB→11KB）。 |
| `terminal echo >>` | `fact_store.jsonl` 单行追加 | Windows: 外层单引号给 bash，JSON 内用双引号。写完后 `read_file` 验证最后一行完整。 |
| `fact_store(action='add')` | 温层事实写入 | 处理 dedup/trust，但 tool 可能不在 cron 上下文中。如有则优先用。 |
| ❌ `patch()` | 含中文或多个模板化字符串的文件 | 不可靠。失败后立刻转用 `write_file` 或 `echo >>`，不要重试。 |

**`write_file` 全量重写模式（MEMORY.md 追加专用）：**

当 `patch` 因非唯一匹配失败时：

```
1. read_file(MEMORY.md)           # 获取当前全文
2. 拼接到完整内容末尾             # new_full = current + "\n§\n## ... auto-learned: ..."
3. write_file(MEMORY.md, new_full) # 一次写入
```

**2026-05-16 事故流程（两次不同的失败）：**
1. `patch(fact_store.jsonl)` 因 CJK 编码偏差失败 → 转用 `echo >>` 成功
2. `patch(MEMORY.md)` 因多段相同模板（`|- Platform:` 出现 4 次）失败 → 转用 `write_file` 全量重写成功

**教训：** 写入 MEMORY.md 和 fact_store.jsonl 的流程应该是 **先试最高效的工具，一旦失败立刻切换到另一种方法，不要重试同一种失败路径**。`patch` 失败后尝试 `write_file(完整文件)` 或 `echo >>`，而不是再试一次 `patch`。

**2026-05-16 另一个 8 次失败教训（跨 session）：** 如果 `fact_store` tool 不在可用工具列表中，不要连续尝试 8 次 memory tool 调用。第一次失败后就改走文件直写路径。重复尝试只会浪费工具调用额度。

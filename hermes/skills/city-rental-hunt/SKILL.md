...
2. **Generate search keywords**
   - Run the helper script. **On Windows**, use the full path + Python 3.12 (3.11 has a stdlib mismatch):
     ```bash
     /c/Users/77/.local/bin/python3.12 \
       C:/Users/77/AppData/Local/hermes/skills/city-rental-hunt/scripts/keyword_plan.py \
       --city 北京 \
       --zones "北苑,霍营,清河" \
       --budget "6000-9000" \
       --rooms "两居" \
   ...
   **⚠️ Prerequisite**: TikHub must be available for the search steps (Xiaohongshu/Douyin). If TikHub is not installed, fall back to web search (Baidu, Bing) or direct platform search. The workflow is still valuable without TikHub — just fewer platform options.
...
- **keyword_plan.py fails on Windows with `SRE module mismatch`**: The Python 3.11 in the default PATH has a stdlib corruption. Fix: use `/c/Users/77/.local/bin/python3.12` explicitly, or fall back to `D:/Code/environment/python/python.exe`.
- **TikHub not installed**: The skill still works without it — use Baidu/Bing web search as fallback. TikHub is preferred but not required.
- **3000 CNY budget in 上地 core area**: At this price point, a full independent 1BR with private bath is unrealistic. Recommend shifting to 合租单间 or expanding radius to 昌平线 northern stations (沙河, 巩华城).
- **Short Chinese queries beat long NL queries**: On TikHub/XHS, short phrases like `上地 整租 一居` outperform verbose natural language strings.

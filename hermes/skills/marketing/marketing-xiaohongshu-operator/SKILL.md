...
日常监控小红书热点时，经常遇到海外 IP 被风控拦截的问题。参考 `references/xiaohongshu-data-access.md` 获取具体方案。

### 推荐的数据获取方式

1. **46.la 热榜**：直接导航到 `https://www.46.la/tool/xiaohongshu-hot` 即可查看实时热榜 TOP20，无需登录
2. **Bing 中文搜索**：用 `cn.bing.com` 搜索"小红书 热门话题 + 日期"获取第三方文章
3. **千瓜数据**（付费）：专业的小红书数据分析平台

### 从热榜数据中提取选题方向

拿到热榜话题后，分析维度：
- **内容类型**：旅行/摄影、美食、DIY、美妆、传统文化等
- **情绪标签**：治愈（日出/海景）、搞笑（新型遛狗法）、干货（拍照姿势）
- **时间/季节信号**：夏日出行、海边日落等应季主题
- **文化切入点**：古诗词、国风等持续热门方向

### 热榜数据解析示例（2025-05-15 数据）

| 排名 | 话题 | 热度 | 类型 |
|------|------|------|------|
| 1 | 用万能旅行拍照姿势美美出片 | 909.9万 | 旅行/拍照干货 |
| 2 | 耗时三年拍下古诗词里的中国 | 900.4万 | 传统文化/摄影 |
| 3 | 我拍到了海鸥雨 | 880.5万 | 自然摄影 |
| 7 | 拼豆上也可以作画了 | 827.8万 | DIY 手工 |
| 14 | 我创造了新型遛狗法 | 726.6万 | 宠物/创意 |

...
**多测试入口项目：**
```
✅ 发现项目同时有自定义 runner 和 pytest → 两者都跑 → 分别报告结果
❌ 只跑 pytest 就宣称整个项目通过，或只跑自定义 runner 就忽略标准测试发现
```

**Python 语法/警告面：**
```
✅ 修改 Python 后可用 compileall 或等价命令补一层语法/警告检查，尤其当测试输出暴露 SyntaxWarning 时
❌ 测试通过但留下由本次修改触发的 warning，当作“无所谓”
```

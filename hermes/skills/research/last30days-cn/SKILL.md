...
### 步骤 1: 运行搜索引擎（Python 脚本方式）

如果环境满足 Python 3.12+ 且已安装依赖（playwright, jieba），优先执行：
...
1. **Bing 国内版**（替代百度，数据中心的 IP 可正常访问）
   ```
   browser_navigate(url="https://cn.bing.com/search?q={中文查询}&setlang=zh-Hans")
   ```
   解析搜索结果页面的标题、摘要和链接。对每个有价值的链接，可以再打开详细内容。

2. **小红书内容**：使用 curl + 手机 UA 绕过反爬（详见 `references/chinese-platform-access.md`）

3. **知乎/微博内容**：通过 Bing 搜索 `site:zhihu.com {topic}` 或 `site:weibo.com {topic}` 索引内容

4. **跨平台交叉验证**：对同一主题从多个搜索结果中提取信息，标注来源

> ⚠️ 技巧参考：`skill_view(name='last30days-cn', file_path='references/chinese-platform-access.md')` 内有各平台反爬绕过方法
